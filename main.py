...
mushrooms = mushrooms_svm.drop(columns=["class"])  # drop bc we're going to predict this
df = pd.concat([input_df, mushrooms], axis=0)  # kết hợp các tính năng đầu vào vào tập dữ liệu
###################
...
dp = dp[:1]
load_clf = pickle.load(open("mushrooms_svm.pkl", "rb"))#đọc mô hình phân loại đã lưu
...
with col1:
    ...
    if prediction == 0:
        st.success("Nấm của bạn có thể ăn được  :relaxed:")
        st.balloons()
    else:
        st.error("Nấm của bạn độc :cry:")
